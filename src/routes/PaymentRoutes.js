...
router.post('/create-checkout-session', AuthAccountMiddleWare, async (req, res) => {
  ...
  try {
    let CommissionCode = req.body.commissionCode;
    let CommissionCodeData = CommissionCode
      ? await PromoCodeTrackingCollection.findOne({ code: CommissionCode })
      : null;

    let CommisionCodeOwner = CommissionCodeData
      ? await UserCollection.findOne({ username: CommissionCodeData.createdBy })
      : null;
    ...
    let sessionConfig = {
      ...
      metadata: {
        refferedBy: CommissionCodeData?.createdBy || '',
    ...
    // if commission code is valid and active, include transfer data
    if (CommissionCodeData?.isActive && CommisionCodeOwner?.stripeAccountId) {
      ...
      // track usage of the commission code
    ...
    await UserCollection.updateOne(
      { _id: req.user._id },
      { $set: { PaymentSession: session.id } }
    );

    return res.status(200).json({ url: session.url, sessionId: session.id });
  } catch (err) {
    console.error('Stripe error:', err);
    return res.status(500).json({ error: 'Stripe session failed', raw: err.message });
  }
});
...
router.get('/check-session', async (req, res) => {
  ...
  try {
    ...
    if (session.payment_status === 'paid') {
      ...
      const token = tokenHeader ? tokenHeader.replace(/Bearer\s/i, '').trim() : UserData.apikey;

      const usedComissionCodes = UserData.CommissionCodeUsed || {};
      const ComissionCodeCheck = usedComissionCodes[promoCode];

      if (ComissionCodeCheck && refferedBy) {
        ...
        if (RefferedUser && RefferedUser.stripeAccountId) {
          ...
          try {
            ...
            console.error(`Stripe transfer tracking failed: ${error}`);
          }
        }
      }
    ...
    return res.status(500).json({ error: 'Failed to retrieve session', raw: err.message });
  }
});
